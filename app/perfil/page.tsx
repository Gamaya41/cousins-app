...
export default function Perfil() {
  ...
  // 1. ARMAZENA AS COORDENADAS DO RECORTE
  ...
  // 2. CAPTURA OS PIXELS QUANDO VOCÊ PARA DE MOVER A FOTO
  ...
  // 3. FUNÇÃO QUE "FOTOGRAFA" O RECORTE E SALVA
  const gerarFotoCortada = async () => {
    ...
    try {
      ...
      // Desenha apenas a parte que você centralizou
      ctx?.drawImage(
        image,
      ...
      localStorage.setItem('userPhoto', fotoFinal); // Salva o recorte definitivo
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white flex flex-col items-center pb-32 font-sans overflow-x-hidden relative">
      ...
      {/* MODAL DE CENTRALIZAÇÃO (Z-INDEX ALTO) */}
      {imagemTemporaria && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex flex-col items-center justify-center p-6 backdrop-blur-md">
          ...
          <div className="relative w-full max-w-sm h-80 bg-zinc-900 rounded-3xl overflow-hidden border border-white/10">
            <Cropper
              ...
              onCropComplete={onCropComplete} // ESSENCIAL PARA O RECORTE FUNCIONAR
              cropShape="round"
              showGrid={false}
            />
          </div>
          ...
          <div className="w-full max-w-sm mt-8 space-y-6">
            ...
            <div className="flex gap-4">
              <button onClick={() => setImagemTemporaria(null)} className="flex-1 bg-zinc-900 border border-white/5 py-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 active:scale-95 transition-all">
                <X size={16} /> Cancelar
              </button>
              <button onClick={gerarFotoCortada} className="flex-1 bg-yellow-600 text-black py-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 active:scale-95 transition-all">
