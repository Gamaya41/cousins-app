...
import { Trophy, Medal, ArrowUp, ChevronLeft, Star } from 'lucide-react';
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white flex flex-col items-center overflow-x-hidden font-sans">
      ...
      <div className="w-full max-w-md flex flex-col relative">
        ...
        {/* 1. NAVEGAÇÃO SUPERIOR - BOTÃO VISÍVEL E ESTILIZADO */}
        <div className="fixed top-6 left-6 z-50">
          <Link 
            href="/" 
            className="flex items-center justify-center w-12 h-12 bg-zinc-900/80 backdrop-blur-md rounded-2xl border border-white/10 text-white shadow-2xl active:scale-90 transition-all hover:bg-zinc-800"
          >
            <ChevronLeft size={24} />
          </Link>
        </div>

        {/* 2. TÍTULO CENTRALIZADO COM ESPAÇAMENTO SEGURO */}
        <header className="w-full text-center mt-20 mb-10 px-6">
        ...
        {/* 3. PÓDIO - COM AJUSTE DE ALTURA PARA NÃO CORTAR O TROFÉU */}
        <section className="w-full px-4 mb-14">
          <div className="flex justify-center items-end gap-2 h-52 relative">
            ...
            <div className="flex flex-col items-center flex-1">
              <div className="w-14 h-14 rounded-full bg-zinc-900 border-2 border-zinc-500 mb-2 flex items-center justify-center text-zinc-500 font-black italic text-xs">#02</div>
            ...
            {/* Terceiro Lugar */}
            <div className="flex flex-col items-center flex-1">
              <div className="w-14 h-14 rounded-full bg-zinc-900 border-2 border-orange-700 mb-2 flex items-center justify-center text-orange-700 font-black italic text-xs">#03</div>
          ...
          {lideres.map((socio) => (
            <div 
              ...
              className={`p-5 rounded-[2.5rem] border flex items-center justify-between transition-all ${
                socio.isMe 
                  ? 'bg-gradient-to-r from-yellow-500 to-yellow-600 border-transparent shadow-xl scale-[1.03]' 
                  : 'bg-zinc-900/40 border-white/5'
              }`}
            >
              <div className="flex items-center gap-4">
                <span className={`font-black italic text-lg ${socio.isMe ? 'text-black' : 'text-zinc-600'}`}>
                  #{socio.rank < 10 ? `0${socio.rank}` : socio.rank}
                </span>
                <div>
                  <p className={`font-black uppercase italic text-sm tracking-tight ${socio.isMe ? 'text-black' : 'text-white'}`}>
                    {socio.nome}
                  </p>
                  <div className="flex items-center gap-1">
                     <Star size={10} fill={socio.isMe ? 'black' : '#eab308'} className={socio.isMe ? 'text-black' : 'text-yellow-500'} />
                     <p className={`text-[9px] font-bold uppercase ${socio.isMe ? 'text-black/60' : 'text-zinc-500'}`}>
                       {socio.pontos} PONTOS
                     </p>
                  </div>
                </div>
              ...
              {socio.isMe && (
                <div className="bg-black/10 p-2 rounded-full">
                  <ArrowUp size={16} className="text-black animate-bounce" />
                </div>
              )}
            </div>
          ))}
        ...
        {/* FOOTER */}
        <footer className="mt-12 px-8 text-center pb-20">
          <p className="text-[9px] font-black uppercase text-zinc-600 mb-6 tracking-[0.2em] italic">
            Faltam 650 pontos para o Top 3!
          </p>
          <Link href="/como-ganhar" className="block w-full">
            <button className="w-full bg-zinc-900 border border-white/5 py-5 rounded-[2rem] font-black uppercase italic text-[11px] tracking-widest active:scale-95 transition-all text-zinc-400">
